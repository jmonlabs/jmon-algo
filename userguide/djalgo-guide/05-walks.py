...
def _(mo):
    ...
    Random walks are available in Djalgo in file walk.pywalk.py. From there, there are two options to generate walks. The first one, Cha∈Chain, is a classic random walk, and the second, Ker≠lKernel, is a Gaussian process generator, which will be discussed later in this section.

    The random walk generator takes several of arguments. The walkran≥walk_range argument takes a list of the minimum and maximum numbers to confine the walk (to create a *bounded* random walk). For instance, if you aim at restraining the walk between -10 and 10, you would enter walkran≥=[-10,10]walk_range=[-10, 10]. The walkprobabilitywalk_probability can take a list of numbers, for instance [-1,0,1][-1, 0, 1] if you aim at generating steps of -1, 0 and 1. It can also take a [Scipy probability distribution](https://docs.scipy.org/doc/scipy/reference/stats.html) function, for instance scipy.stats.∥loc=0,sca≤=1∥scipy.stats.norm(loc=0, scale=1), where locloc and sca≤scale are the mean and the standard deviation. The ≤n>hlength argument specifies the number of steps. The round→round_to argument, with default No≠None, rounds stepping to a given decimal.

    Djalgo has the ability to randomly branch a walk. When a walk is branched, a new walk is created and wanders on it own. The probability to branch is specified from 0 to 1 in the branχngprobabilitybranching_probability argument. Conversely, merg∈gprobabilitymerging_probability is the probability to merge random walks when they collide. If round→round_to is not triggered on, there is virtually no chance that branches collide.
    """)
    return
...
def _(mo):
    mo.md(r"""
    I'd like to generate a random walk through pitches from C2C2 to C8C8, starting at C4C4. MIDI pitch numbers can be recovered using Djalgo's cde_to_midi utility function.
    """)
    return
...
def _(mo):
    mo.md(r"""
    The rwrw is a random walk object, which can be used to generate many random walks, although only one will be generated in this example.
    """)
    return
...
def _(mo):
    mo.md(r"""
    rwsequencesrw_sequences contains 5 branches. If you examine the object, you will see lists beginning with No≠None values, which are generated when a new branch is created to make sure all branches are generated with the same length. At each step, if branching returns TrueTrue with probability branχngprobabilitybranching_probability, a new list is created, and the random walk becomes a list of lists, and all values of the new list happening before the branch are set to No≠None. Each branch then has its own random walk, and can branch out. If two or more branch collide to the same values, and if merging if TrueTrue according to probability merg∈gprobabilitymerging_probability, lists are closed with No≠None values to the following notes, except the branch with the lower index, on which the other branches are merged. The merged branch continues the random walk, and can branch again.
    """)
    return


@app.cell
def _(go, rw_sequences):
    _fig = go.Figure()
...
def _(mo):
    mo.md(r"""
    Seven tracks have been generated. I can generate different rhythms from draws from a single list. I set the seed at the beginning of the block, and not in the .random().random() method to make sure that rhythms are not the same for all tracks, but remains the same each time I run the block. Setting seed is very useful to assures that random processes provide the same results, hence avoid losing an interesting pattern because it was random and can't be recovered.
    """)
    return


@app.cell
def _(dj, g_major, random, rw_index):
    random.seed(123)
    _measure_length = 3
    rw_notes_tuned = []
...
def _(mo):
    mo.md(r"""
    For unfitted GPs, the covariance function is created but Djalgo's rbfker≠lrbf_kernel function, as known as the kernel, with two properties.

    - The ≤n>hsca≤length_scale parameter describes the inertia of the covariance. Larger lengths create longer waves.
    - The multiplier of the kernel, often referred as $\alpha$ in textbooks, describes the amplitudeamplitude of the wave.

    The following plot was generated from djalg⊙walk.Ker≠ldjalgo.walk.Kernel.
    """)
    return


@app.cell(hide_code=True)
def _(dj, go, make_subplots, np):
    ...
    _unique_d = list(set(_kernel_walks["d"]))
    _n_rows = (len(_unique_d) + 2) // 3
    _fig = make_subplots(rows=_n_rows, cols=3, subplot_titles=_unique_d)
    _colors = ["#1f77b4", "#ff7f0e", "#2ca02c"]

    for _i, _d in enumerate(_unique_d):
        _row = _i // 3 + 1
        _col = _i % 3 + 1

        for _sample, _color in zip(
            ["sample: 0", "sample: 1", "sample: 2"], _colors
        ):
            _mask = np.array(
                [
                    (_dval == _d) and (_sval == _sample)
                    for _dval, _sval in zip(
                        _kernel_walks["d"], _kernel_walks["sample"]
                    )
                ]
            )
            _x_filtered = [_x for _x, _m in zip(_kernel_walks["x"], _mask) if _m]
            _y_filtered = [_y for _y, _m in zip(_kernel_walks["y"], _mask) if _m]
            _fig.add_trace(
                go.Scatter(
                    x=_x_filtered,
                    y=_y_filtered,
    ...
    _fig.update_layout(
        height=800, width=800, showlegend=True, title_text="Kernel Walks"
    )
    _fig.update_yaxes(matches="y")
    _fig
    return
...
def _(mo):
    mo.md(r"""
    The next block does three things. First, the dj.walk.Ker≠ldj.walk.Kernel generates a number of ntracksntracks samples from a Gaussian process. As for the second part, because the numbers generated are indexes and not pitches, the forfor loops, needs them to be integer values between (included) 0 to the rank of the last index of the scale we want to pick pitches from. Finally, the tracks are plotted. Plots are useful to calibrate amplitudeamplitude and ≤n>hsca≤length_scale. Different combinations of amplitudeamplitude and ≤n>hsca≤length_scale were tried to obtain signals that are sufficiently wavy in the x direction (length scale) and ranging through the indexes allowed in the scale list.
    """)
    return


@app.cell
def _(blues_scale_length, dj, go):
...
@app.cell
def _(b_blues_scale, blues_length, blues_ntracks, blues_tracks_i):
    blues_track_chords = [
...
def _(mo):
    mo.md(r"""
    Finally, isorhythms are applied to generate one track of chords.
    """)
    return


@app.cell
def _(blues_rhythm, blues_track_chords, dj):
...
def _(dj):
    ...
    return d_major, pachelbel_progression_iso


@app.cell(hide_code=True)
def _(mo):
    mo.md(r"""
    > **Note**. The code under #F∈dthechors∈dmajor# Find the chors in d_major is really helpful to add *voicing* to a progression.
    """)
    return
...
def _(mo):
    mo.md(r"""
    We printed the chords, but our constrained will really be pitch indexes of the fundamentals. To model this, we add a datadata argument to our djalg⊙walk.Ker≠ldjalgo.walk.Kernel object in the form of a Numpy array with time (offset) in the first column, and pitch index in the second
    """)
    return


@app.cell
def _(d_major, np, pachelbel_progression_iso):
    data = np.array(
        [
...
def _(mo):
    mo.md(r"""
    When data are provided, Djalgo interpolates a number of ≤n>hlength data points on the range of the data, fits the model, then export results in time and model predictions.
    """)
    return
...
def _(data, dj):
    ...
    return ntracks, pachelbel_tracks_p, pachelbel_tracks_t


@app.cell
def _(data, go, pachelbel_tracks_p, pachelbel_tracks_t):
    _fig = go.Figure()
...
@app.cell
def _(np, pachelbel_tracks_p, pachelbel_tracks_t):
...
def _(mo):
    mo.md(r"""
    So, for each track, let's create a random rhythm, interpolate indexes to the offsets, find the indexes on the dmajord_major scale, and assemble the tracks.
    """)
    return
...
@app.cell
def _(dj, pachelbel_progression_iso, tracks):
...
def _(mo):
    ...
    Ce≤stialBodyCelestialBody and SolarSystemSolarSystem classes in the walk module simulates the distance and angles of celestials bodies in a solar system, given their orbital radius, orbital speed and the celestial body it is orbiting around. I created celestial walks after watching a [video from Marc Evanstein](https://www.youtube.com/watch?v=wn3td88w19E), who explained concepts of circular music. Let's start by creating the smallest celestial body of our solar system.
    """)
    return
...
@app.cell
def _(celestial_results, celestial_times, go, make_subplots):
...
def _(mo):
    ...
    Just like celestial walks, I identified prospects of the Fibonacci sequence in music by watching [a video from Marc Evanstein](https://www.youtube.com/watch?v=mE3r5E6794U). In Djalgo, dj.utils.fibonai()dj.utils.fibonacci() is not a walk *per se*, but an iterator. This means that calling ≠xtnext to it will return the next number of the sequence. An iterator is useful when it is usually used in a loop, as usually done with Fibonacci numbers. Don't worry, iterators are quite easy to use.

    In the following code block, I instanciate the iterator to the name fibonaifibonacci, initiate a liste to store *degrees* (placement in a scale), then ask 30 times to add the next Fibonacci number to it.
    """)
    return
...
def _(mo):
    mo.md(r"""
    Woah, the sequence really goes wild! Fortunately, we can take tame the Fibonacci sequence with the modulo operator, %%. The modulo is the remainder of a division. For instance,
    """)
    return
...
def _(mo):
    ...
    <div class="main-container">
        <div class="nav-container">
            <a href="https://marimo.io/p/@djalgo/notebook-9pbax5" class="nav-link" target="_top">← previous</a>
            <div class="title">Djalgo's User Guide</div>
            <a href="https://marimo.io/p/@djalgo/notebook-1ehwep" class="nav-link" target="_top">next →</a>
        </div>
        <div class="toc-container">
            <ul>
                <li><a href="https://marimo.io/p/@djalgo/notebook-a0jyt6" target="_top">01. Installation</a></li>
                <li><a href="https://marimo.io/p/@djalgo/notebook-9pbax5" target="_top">02. Harmonies</a></li>
                <li><a href="https://marimo.io/p/@djalgo/notebook-6hmu8z" target="_top"> 03. Loops</a></li>
                <li><a href="https://marimo.io/p/@djalgo/notebook-1ehwep" target="_top"> 04. Minimalism</a></li>
                <li><a href="https://marimo.io/p/@djalgo/notebook-xsbu6g" class="active" target="_top">> 05. Walks</a></li>
                <li><a href="https://marimo.io/p/@djalgo/notebook-rkm517" target="_top">06. Fractals</a></li>
                <li><a href="https://marimo.io/p/@djalgo/notebook-yefoq7" target="_top">07. Genetic algorithms</a></li>
                <li><a href="https://marimo.io/p/@djalgo/notebook-sbmu0j" target="_top">08. AI</a></li>
            </ul>
        </div>
...
def _():
    ...
    return go, make_subplots, math, mo, np, random, scipy, sp
