...
const jm = require("./dist/jmon.esm.js").default ||
  require("./dist/jmon.esm.js");
...
console.log("🎵 Testing REAL jmon-algo articulation enrichment API\n");
...
try {
  ...
  console.log(
    "   Articulated notes:",
    articulatedNotes.map((n) => `pitch ${n.pitch}`),
  );
} catch (error) {
...
try {
  ...
  if (hasGlissando) {
    console.log(
      "✅ Success! Added glissando articulation with target pitch 62",
    );
  } else {
